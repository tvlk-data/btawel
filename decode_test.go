...
func TestReadPointerReference(t *testing.T) {

	buf := &bytes.Buffer{}
	binary.Write(buf, binary.BigEndian, int64(23))
	...
	var person struct {
		Name    *string `bigtable:"info:name"`
		Address *string `bigtable:"info:address"`
		Age     *int    `bigtable:"info:age"`
	}
	err := btawel.ReadRow(row, &person)

	require.NoError(t, err)
	require.Equal(t, "John", *(person.Name))
	require.Equal(t, int(23), *(person.Age))
	require.Nil(t,  person.Address)
}
